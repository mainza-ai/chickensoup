...
```
src/
├── main.py                 # FastAPI entry point (1611 lines)
...
├── scheduler.py            # Periodic chat-to-wiki background loop (711 lines)
├── tasks.py                # Celery async task definitions (83 lines)
├── agents/
│   ├── orchestrator.py     # pydantic-graph orchestration (263 lines)
...
├── wiki/
│   ├── writer.py           # Wiki page CRUD, cross-referencing, index/log (241 lines)
│   └── __init__.py
├── knowledge_graph/
...
├── ContentView.swift                      # Main orchestrator (496 lines)
├── Models/
...
├── Shared/
│   ├── Services/
│   │   ├── BackendService.swift           # Central service (454 lines)
│   │   ├── GraphService.swift             # KG navigation (211 lines)
│   │   ├── WikiService.swift              # Wiki management
│   │   ├── ChatService.swift              # Chat-to-wiki (136 lines)
│   │   ├── ConfigService.swift            # LLM/quantum config
│   │   ├── LLMDiscoveryService.swift      # LLM provider discovery
│   ├── Networking/
│   │   ├── APIClient.swift                # Actor-based HTTP client
│   │   └── APIModels.swift                # Codable API response models (822 lines)
│   └── DesignSystem/
...
└── Features/
    ...
    │       ├── TimelineNodeView.swift     # Timeline node display
    ...
    │       ├── LoreRepositoryView.swift   # Lore browsing
    │       └── WikiInsightNotificationView.swift # Auto-sliding banner (64 lines)
    ├── Wiki/
    │   └── Views/
    │       ├── WikiBrowserView.swift      # Markdown wiki browsing
    │       └── WikiPageDetailView.swift   # Wiki page detail view
    └── Settings/
